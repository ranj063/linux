...
static int sof_ipc4_load_library(struct snd_sof_dev *sdev, guid_t *widget_uuid)
{
	struct sof_ipc4_fw_data *ipc4_data = sdev->private;
	...
	/* nothing to do if module is built into the base firmware */
	for (i = 0; i < ipc4_data->num_fw_modules; i++)
		if (guid_equal(&ipc4_data->base_fw_module_uuids[i], widget_uuid))
			return 0;

	ret = snprintf(fw_filename, LIBRARY_FILENAME_LEN, "%pUL.bin", widget_uuid);
	if (ret < 0) {
		dev_err(sdev->dev, "failed to create library filename for %pUL\n", widget_uuid);
		return ret;
	}
