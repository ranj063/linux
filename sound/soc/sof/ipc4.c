...
static int sof_ipc4_get_reply(struct snd_sof_dev *sdev)
{
	...
	snd_sof_dsp_mailbox_read(sdev, sdev->host_box.offset, ipc4_reply->data_ptr,
				 msg->reply_size);
				 ...
				 size_t payload_bytes, bool set)
{
	...
	do {
		...
		if (ret < 0) {
			dev_err(sdev->dev,
				"%s: large config %s failed in chunk #%zu (offset %zu): %d\n",
				__func__, set ? "set" : "get", offset / payload_limit,
				offset, ret);
			goto out;
		}
