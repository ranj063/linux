...
static int sof_ipc4_set_get_kcontrol_data(struct snd_sof_control *scontrol, bool set)
{
	...
	if (!swidget || swidget->id != snd_soc_dapm_pga) {
		dev_err(scomp->dev, "Failed to find pga widget for kcontrol %s\n", scontrol->name);
		return -ENOENT;
	}
...
/* TODO: return value from DSP if runtime active */
...
const struct ipc_tplg_control_ops tplg_ipc4_control_ops = {
